    ...
    ngOnInit(): void
    {
        ...
        this._router.events
            ...
            for(let i=0;i<this.navigation.length;i++){
                if(i==2){
                    this.abcd = this.navigation[2].children
                    // alert(JSON.stringify(this.abcd));
                }
            }

            if(this.type=="ContentManager"){
                this.abcd.forEach((item,index)=>{
                    alert(item)

                   if((item.title!='College' &&  item.title!='Events'  && item.title!='Articles' && item.title!='Exams')){
                       this.mydata.push(index)
                   }
               })
            }

            if(this.type=="ContentWriter"){
                this.abcd.forEach((item,index)=>{
            ...
            if(this.type=="College"){
                this.abcd.forEach((item,index, index1)=>{
                   if((item.title!='College' )){
                       this.mydata.push(index)


                   }
                   if (item.id === 'apps.college') {
                    item.children = item.children.filter(child => child.id === 'apps.college.colleges');
                }

               })
            }

            for (let i = this.mydata.length -1; i >= 0; i--){
                this.navigation[2].children.splice(this.mydata[i],1);
            }
