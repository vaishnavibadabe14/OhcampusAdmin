...
export class BlogcategoryComponent implements OnInit {
    displayedColumns: string[] = ['Sr.No', 'catname' , 'status','actions'];
    dataSource : any
    ...
    getCategoryList(){
      ...
      this.campusService.getBlogsCategoryList(this.type,this.page,this.pageSize,this.startNum,this.columnIndex,this.sortValue,this.category.value.search).subscribe((res) =>{
    ...
    addCategory(){
      this._route.navigate(['apps/blog/addblogcategory']);
    }
    ...
    editCategory(catId) {
     ...
     // this._route.navigate(['apps/faqs/addcategory/update/'+ cat_id]);
      this._route.navigate(['apps/blog/addblogcategory/update/'+ cat_id]);
    }
    ...
    deleteCategory(catId){
      let cat_id = catId
        Swal.fire({
          title: 'Are you sure?',
          text: 'You want to delete article category',
          icon: 'warning',
          ...
          if (result.isConfirmed) {
            this.campusService.deleteBlogCategoryDetails(cat_id).subscribe((res) =>{
              if(res.response_message == "Success"){
                this.getCategoryList()
                Swal.fire(
                  'Deleted!',
                  'Your article category has been deleted.',
                  'success'
                );
              }
