...
@Component({
  ...
  styleUrls: ['./eventcategory.component.scss']
})
export class EventcategoryComponent implements OnInit {
  ...
  addCategory(){
    ...
    this._route.navigate(['apps/event/addeventcategory']);
  }
  ...
  editCategory(catId) {
    let cat_id = catId;
    this._route.navigate(['apps/event/addeventcategory/update/'+ cat_id]);
  }
  ...
  deleteCategory(catId){
    let cat_id = catId
      Swal.fire({
        title: 'Are you sure?',
        text: 'You want to delete event category',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
          this.campusService.deleteCategoryDetails(cat_id).subscribe((res) =>{
            if(res.response_message == "Success"){
              ...
              Swal.fire(
                'Deleted!',
                'Event category has been deleted.',
                'success'
              );
            }
