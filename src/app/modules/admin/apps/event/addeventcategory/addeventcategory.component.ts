...
@Component({
  ...
  styleUrls: ['./addeventcategory.component.scss']
})
export class AddeventcategoryComponent  implements OnInit {
  ...
  if (routeParams.catId) {
    ...
    addCategory(){
      ...
      this.campusService.insertCategoryDetails(catname,topmenu,
        ...
        Swal.fire({
          text:  'Event category added successful',
          icon: 'success',
          ...
          if (result.isConfirmed) {
            this._route.navigate(['apps/event/eventcategory']);
          }
        });
        }else{
          this.addLoader = false
  ...
  updateCategory(){
    ...
    this.campusService.updateCategoryDetails(catname,topmenu,
        ...
        Swal.fire({
          text:  ' Event category updated successful',
          icon: 'success',
          showCancelButton: false,
          confirmButtonColor: "#3290d6 !important",
          confirmButtonText: 'Ok'
